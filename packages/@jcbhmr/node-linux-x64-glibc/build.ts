...
const response = await fetch(`https://nodejs.org/dist/v${nodeVersion}/node-v${nodeVersion}-linux-x64.tar.xz`)
...
await $({ verbose: "short" })`tar -xJf ./dist/node.tar.xz -C ./dist/ node-v${nodeVersion}-linux-x64/LICENSE node-v${nodeVersion}-linux-x64/bin/node --strip-components=1`
